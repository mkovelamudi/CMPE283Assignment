...
*/
struct capability_info procbasedprimary[22] =
{
...
*/
struct capability_info exitsbased[14] =
{
...
detect_vmx_features(void)
{
	...
	/* Procbased primary controls */
        ...
        pr_info("Procbased primary Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(procbasedprimary, 22, lo, hi);
	...
	/* Procbased secondary controls */
        ...
        pr_info("Exit Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(exitsbased, 14, lo, hi);
